...
const toLastNames = people => 
  ...
  people.map(value => `${value.firstName} ${value.lastName}`);
...
const addPurchases = (arr) => 
  ...
  arr.reduce((accumulator, object) => + (accumulator + object.purchasePrice), 0);
...
const countNumberOfElements = (arr) => 
  ...
  arr.reduce(accumulator => accumulator + 1);
...
const returnNames = (arr) => 
  ...
  arr.reduce((accumulator, object) => accumulator.push(object.name) && accumulator, []);
...
const reversedString = (str) => {
  ...
  return str.split('').reduce((reverse, value,) => value + reverse);
};
