...
const firstLetters = (arr) => {
  ...
  const newArray = arr.map(value1 => value1.split(''));
  const newMap = newArray.map(value2 => value2[0]);
  return newMap;
};
...
const findHappiness = (arr) => {
  ...
  const newArray = arr.filter(value => value.includes(')'));
  return newArray;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  const regex = /\s|-|\(|\)/g;
  const newArray = arr.map(value => value.replace(regex, ''));
  return newArray;
};
...
const onlyOddChars = (str) => 
  ...
  str.split('').filter((a,b) => b % 2 === 1).join('');
