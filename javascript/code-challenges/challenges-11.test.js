...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    let newArr = input[i];
    newArray.push(newArr.filter(number => ((number % 5 === 0) && typeof (number) === 'number')).map(number => Math.pow(2, number)));
  }
  return newArray;
};
...
let findShortest = (data) => {
  ...
  let value = data.filter( character => character.height).reduce((a, b) => (b.height > a.height ? b : a));
  return value.name;
};
